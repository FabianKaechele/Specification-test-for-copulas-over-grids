"""   Performs a calculation of empirical upper tail
https://publications.waset.org/9997108/pdf
https://www.econstor.eu/bitstream/10419/29588/1/614056446.pdf
Input: empirical return vectors
Output: csv with empirical upper tail dependece.
    ...
    for Industry2 in Industry:
        if Industry1!=Industry2:
            if Industry.index(Industry1) < Industry.index(Industry2) :
                ...
                uppertotal=np.zeros(shape=runs)
                ...
                while i<(runs*rolling):
                    ...
                    # Calculate Upper Tail Dependece

                    ahelp=np.where((first[:,0]< (1-math.sqrt(size)/size) )&(first[:,1]< (1-math.sqrt(size)/size) ),1,0)
                    a=sum(ahelp)/size
                    upper=2-math.log(a)/math.log(1-math.sqrt(size)/size)
                    uppertotal[h]=upper
                ...
                # Save csv
                name = str(Industry1) + '_'+str(Industry2)+'_tail.csv'
                np.savetxt(name, uppertotal, delimiter=",")
