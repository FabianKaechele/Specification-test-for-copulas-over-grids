...
""" use the functions out of this file to perform the baseline test, without the selection step and Benjamini-Hochburg adjustment
                        """
import numpy as np
import scipy
...
#Bonferroni Adjustment alpha
    ...
    for i in range(0,Fenster):
        for j in range (0,Fenster):
           ...
           # p_value1 = scipy.stats.f.ppf(1-alphabonf, 1, min(n1, n2) - 5)

            #if T>p_value1:
            #if p_value > 1-alphabonf:
                #print(i,j,'ablehnen {}'.format(p_value))

            t1[i,j,h]=p_value
           # else:
                #t1[i, j,h] = p_value1
                #print(i,j,'annehnen {}'.format(p_value))
#    print ()
    ...
    for i in range(0, Fenster):
        for j in range(0, Fenster):
           ...
           # if T > p_value1:
            #if p_value > 1-alphabonf:
                #print(i, j, 'vs',j,i, 'ablehnen {}'.format(p_value))
                #Total = 'Nullhypothese wird abgelehnt!'
            t2[i,j,h]=p_value

            #else:
                #t2[i, j,h] = p_value1
                #print(i, j, 'vs',j,i, 'annehnen {}'.format(p_value))

    # Trage Ergebnis in Ergebnisvektor h ein
    #build array with p-values
    A=np.zeros(2*(Fenster**2)-Fenster)
    k=0
    for i in range(0, Fenster):
        for j in range(0, Fenster):
            A[k]=t1[i,j,h]
...
# Abändern T1 für empirische Daten. ggf auskommentiriern
    k=0
    for i in range(0, Fenster):
        for j in range(0, Fenster):
            t1[i,j,h]=pvals[k]
            if  reject[k]==True:
                ...
                t1[i, j, h]=0
            k=k+1
    #wieder in T scchreiben
    #t1[:,:,h]=np.where(t1[:,:,h]==True,1,0)
    #Gesamtvektor bauen
    if any(reject):
        abgelehnt[h]=1
...
def Chitest(alphatotal, p1, p2, n1, n2, Fenster, h, abgelehnt):
    """    Berechnet Teststatistik fuer jedes Fenster und vergleicht mit kritischem Wert
            Erzeugt Detailansicht t1/t2 je Testdurchlauf mit Ablehnungswahrscheinlichkeit je Fenster - nach fdr correction
    ...
    if T!=None:
        p_value = scipy.stats.chi2.cdf(T, (((Fenster-1)**2)-2))
