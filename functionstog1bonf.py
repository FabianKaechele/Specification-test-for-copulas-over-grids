# Fabian Kaechele
""" functions for basic overall test with bonferroni adjustment
                        """
import numpy as np
import scipy
...
# Schieben des Grid
    ...
    # uu, vv = one.generate_uv(UU, WW, n1)
    runs=0
# Grafik erzeugen
#     if runs==1 and n1==1000:
#         fig = plt.figure()
#         fig.add_subplot(2,2,1)
#         plt.scatter(u,v,marker='.',color='blue',s=0.1)
#         plt.ylim(0,1)
#         plt.xlim(0,1)
#
#         x = u
...
# #    aa = np.transpose(np.vstack([uu, vv]))
...
def test(alphatotal,first,second,n1,n2,t1,t2,Fenster,h,abgelehnt):
    """ Berechnet Teststatistik fuer jedes Fenster und vergleicht mit kritischem Wert
        Erzeugt Detailansicht t1/t2 je Testdurchlauf mit Ablehnungswahrscheinlichkeit je Fenster - nach bonferoni correction
...
# smalest p-value of test
    ...
    # position of this value
        Ghelp=list(A)
        G[r-2]=Ghelp.index(np.amin(A))



#Gesamtvektor bauen
    DD=None
    if any(B*(2*(Fenster**2)-Fenster)<alphatotal):
        abgelehnt[h]=1
