...
def generateCopulas(family1, family2,n1,n2, tau1,tau2,pr1,pr2,df1,df2, mix):
    """ Erzeugt gewuenschte Copulas durch bedinge Inversionsmethode und plottet Copulas
        ...
        Output: first: values of rotated first copula u,v
    ...
    # drehen der Copula
    uu = u * np.cos(0.5 * math.pi) - v * np.sin(0.5 * math.pi) + 1
    vv = u * np.sin(0.5 * math.pi) + v * np.cos(0.5 * math.pi)
...
# #    Grafik erzeugen
    if runs==1 and n1==1000:
        ...
        plt.scatter(uu, vv, marker='.', color='blue', s=1)
        plt.ylim(0, 1)
        plt.xlim(0, 1)
...
#
        if family1 == 'frank':
            ...
            plt.title('Exchangeable Copula', **font)
        elif family1 == 'gumbel':
            plt.title('Gumbel Copula \u03C4 = {}'.format(round(one.tau, 2)), **font)
        elif family1 == 'clayton':
            plt.title('Rotated Clayton Copula \u03C4 = {}'.format(round(one.tau, 2)), **font)
            plt.title('Non-exchangeable Copula', **font)
        elif family1 == 'normal':
    ...
    first = np.transpose(np.vstack([uu, vv]))
    ...
    second = np.transpose(np.vstack([uu, vv]))
